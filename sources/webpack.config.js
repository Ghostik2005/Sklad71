...
module.exports = function(env) {
	...
	var config = {
		...
		output: {
			...
			chunkFilename: `${pack.name}.bundle.js`
		},
		module: {
			rules: [
				{
		...
		plugins: [
			...
			new HtmlWebpackPlugin({
				hash: true,
				filename: (asmodule || standalone) ? `${pack.name}.html` : "index.html", //имя файла
				templateParameters: {
					name: `${pack.name}`,
					title: "Склад71",
					ver: new Date().toISOString().replace(/-/g, "").replace(/:/g, "").replace(/\./g, ""),
				},
				template: path.resolve(__dirname, (asmodule || standalone) ? "./sources/html/empty.ejs":"./sources/html/index.ejs") , // имя шаблона
				inject: false,
			})
		],
		devServer:{
