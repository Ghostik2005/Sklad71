...
class BOTClient(object):
    ...
    def send(self, name, args, kwargs, sync=True, bot_id=None, botname=None):
        ...
        r.error = None
        i = id(r)
        data = ('PUB %s %s.%x %s\r\n' % (bot_id if bot_id else self.bot_id, self.cli_id, i, len(data))).encode('utf8') + data + b'\r\n'
        self._sync[i] = r
        with self._w_lck:
            self._sock.sendall(data)
        #print(dir(r))
        ...
        if r.wait(_wait1):
            ...
            if sync:
                raise ServiceError('Service <%s> not found' % (botname if botname else self._botname))
            else:
...
if __name__ == '__main__':
    ...
    c1 = c('mybot.conf11', bot=True, sync=(1, 10))
    #pprint(c.system.listMethods())

    #pprint(c.system.listMethods('time'))
